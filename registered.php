<!DOCTYPE HTML>
<!--
	Alpha by HTML5 UP
	html5up.net | @ajlkn
	Free for personal and commercial use under the CCA 3.0 license (html5up.net/license)
-->
<html>
	<head>
		<title>Registered</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
		<link rel="stylesheet" href="assets/css/main.css" />
	</head>
	<body class="is-preload">
		<div id="page-wrapper">

			<!-- Header -->
				<header id="header">
					<h1><a href="index.php">Blogs</a> :P</h1>
					<nav id="nav">
						<ul>
							<li><a href="index.php">Home</a></li>
             <li><a href="article2.php?name=food">美食</a></li>
              <li><a href="article2.php?name=travel">旅遊</a></li>
              <li><a href="article2.php?name=mood">心情</a></li>
              <li><a href="article2.php?name=life">生活</a></li>
              <li><a href="article2.php?name=fasion">流行</a></li>
							<li>
								<a href="#" class="icon solid fa-angle-down">More</a>
								<ul>
									<li><a href="contact.php">Contact</a></li>
									<!--<li><a href="elements.html">Elements</a></li>
                  <li><a href="generic.html">文章</a></li>
									<li>
										<a href="#">Submenu</a>
										<ul>
											<li><a href="#">Option One</a></li>
											<li><a href="#">Option Two</a></li>
											<li><a href="#">Option Three</a></li>
											<li><a href="#">Option Four</a></li>
										</ul>
									</li>-->
								</ul>
							</li>
							<li><a href="sign.php" class="button">Sign Up</a></li> <!--登入-->
						</ul>
					</nav>
				</header>

			<!-- Main -->
				<section id="main" class="container">
					<header>
						<h2>Registered</h2>
						<p>Register an account to write articles</p>
					</header>
					<div class="row">
						<div class="col-12">

							<!-- Text
								<section class="box"> -->
									<h3>Enter your account, password and name to register an account</h3>
									<form action="registered2.php" method="POST">
                  <font color='red'>＊</font>account：<input type="text" name="acc" value="">
                  <font color='red'>＊</font>password：<input type="password" name="pwd" value="">
                  <font color='red'>＊</font>name：<input type="text" name="name" value="">
                  <br>
                  <input type="submit" value="註冊">
                  </form>
									<hr />	
								<!--</section>-->
						</div>
					</div>
          
		<!-- Scripts -->
			<script src="assets/js/jquery.min.js"></script>
			<script src="assets/js/jquery.dropotron.min.js"></script>
			<script src="assets/js/jquery.scrollex.min.js"></script>
			<script src="assets/js/browser.min.js"></script>
			<script src="assets/js/breakpoints.min.js"></script>
			<script src="assets/js/util.js"></script>
			<script src="assets/js/main.js"></script>

	</body>
</html>